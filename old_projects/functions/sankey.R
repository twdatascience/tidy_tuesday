...
#'
#' @export
# modify ggsankey function to have ordered factors if node_order provided
make_long <- function(.df,
  ...
  if (!missing(value)) {
    ...
    out <- .df %>%
      ...
      # dplyr::mutate(x = factor(x, ordered = TRUE),
      #               node = factor(node, ordered = TRUE)) %>%
      dplyr::arrange(.data$..r) %>%
      dplyr::group_by(.data$..r) %>%
      dplyr::mutate(next_x = dplyr::lead(.data$x),
                    next_node = dplyr::lead(.data$node)) %>%
  ...
  if (!missing(node_order) & is.character(node_order)) {
    levels <- factor(unique(out$x), levels = node_order, ordered = TRUE)
  } else {
    levels <- unique(out$x)
  }

  out <- out %>%
    dplyr::mutate(dplyr::across(c(x, next_x), ~ factor(
...
StatSankeyFlow <- ggplot2::ggproto(
  ...
  setup_data = function(data, params) {
    purrr::map_dfr(unique(data$PANEL), ~ {
      ...
      data <- data %>%
        dplyr::mutate(dplyr::across(c(x, next_x), ~
                                      as.numeric(.), .names = ("n_{.col}")))
      ...
      if (!("value" %in% names(data))) {
        ...
        flow_data <- data %>%
          dplyr::mutate(group = 1) %>%
          dplyr::group_by(n_x, node, n_next_x, next_node) %>%
          dplyr::summarise(flow_freq = sum(value, na.rm = TRUE),
                           .groups = "keep") %>%
          dplyr::ungroup()
        ...
        data <- data %>%
          ...
          dplyr::summarise(freq = sum(value, na.rm = TRUE),
                           ,
                           .groups = "keep") %>%
          dplyr::ungroup()
      }
      ...
      if (params$type == "sankey") {
        data <- data %>%
          dplyr::group_by(n_x) %>%
          dplyr::mutate(ymin = ymin - max(ymax) /
                          2,
                        ymax = ymax - max(ymax) /
                          2) %>%
          dplyr::ungroup()
      ...
      data <- data %>%
        dplyr::mutate(xmin = n_x - params$width /
                        2,
                      xmax = n_x + params$width /
                        2)
...
StatSankeyBumpFlow <- ggplot2::ggproto(
  ...
  setup_data = function(data, params) {
    purrr::map_dfr(unique(data$PANEL), ~ {
      ...
      data <- data %>%
        dplyr::mutate(xmin = n_x, xmax = n_x)
...
StatSankeyText <- ggplot2::ggproto(
  ...
  setup_data = function(data, params) {
    purrr::map_dfr(unique(data$PANEL), ~ {
      ...
      data <- data %>%
        dplyr::mutate(x = n_x, y = ymin + (ymax - ymin) /
                        2)
